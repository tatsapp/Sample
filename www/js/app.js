...
$scope.nextSlide=function(){
  ...
  $scope.twitterLogin = function() {     
      $ionicPlatform.ready(function(){
        $cordovaOauth.twitter("EXqH2yY8MHMltdY4olxRb99O1","4XduTuJEu9LA35dxVwdJDtrYxRlJI9Oq4QeZJt9gDp6PZ9hZ8r").then(function(result){
          SQLService.getUserId(result.user_id).then(function(results){
                 MyService.setId(result.user_id);
                if(results.length){
                       $scope.next();
                }
                else
                {
                 ...
                 if(result.birthday)
                   $scope.user_birthday=new Date(result.birthday);            
                else
                   $scope.user_birthday=new Date();
                if(result.gender)
                   $scope.user_gender=result.gender;
                else
                   $scope.user_gender="male";
            var details={name:$scope.user_name,birthday:$scope.user_birthday,gender:$scope.user_gender,id:result.user_id};
    ...
    $ionicPlatform.ready(function(){
      $cordovaOauth.github("0649f1c842d83bb90844","9a16390c8ee067ceb4129e5d86965676130c33ec",["user","repo","gist"]).then(function(result){
        $http.get("https://api.github.com/user?access_token="+result.access_token).then(function(user_detail){
               //alert(JSON.stringify(user_detail.data));
               // 
               SQLService.getUserId(user_detail.data.id).then(function(results){
                 MyService.setId(user_detail.data.id);
                if(results.length){
                       $scope.next();
                }
                else
                {
                 ...
                 if(user_detail.data.birthday)
                   $scope.user_birthday=new Date(user_detail.data.birthday);            
                else
                   $scope.user_birthday=new Date();
                if(user_detail.data.gender)
                   $scope.user_gender=user_detail.data.gender;
                else
                   $scope.user_gender="male";
            var details={name:$scope.user_name,birthday:$scope.user_birthday,gender:$scope.user_gender,id:user_detail.data.id};
            $state.go('profile',details);
          }
       },function(error){
      alert(error);
  ...
  $scope.googleLogin=function(){
    $ionicPlatform.ready(function(){
      ...
      function(result){
        SQLService.getUserId(result.userId).then(function(results){
                 MyService.setId(result.userId);
                if(results.length){
                       $scope.next();
                }
                else
                ...
                if(result.gender)
                   ...
                   $scope.user_gender="male";
            var details={name:$scope.user_name,birthday:$scope.user_birthday,gender:$scope.user_gender,id:result.userId};
            $state.go('profile',details);
          }
       },function(error){
      alert(error);
  ...
  $scope.instaLogin=function(){
    $ionicPlatform.ready(function(){
      $cordovaOauth.instagram("d1aacbd8f58f4ed29b547034cdf72fca",[]).then(function(result){
        ...
        // 
        SQLService.getUserId(res.data.data.id).then(function(results){
                 MyService.setId(res.data.data.id);
                if(results.length){
                       $scope.next();
                }
                else
                {
                 ...
                 if(res.data.data.birthday)
                   $scope.user_birthday=new Date(res.data.data.birthday);            
                else
                   $scope.user_birthday=new Date();
                if(res.data.data.gender)
                   $scope.user_gender=res.data.data.gender;
                else
                   $scope.user_gender="male";
            var details={name:$scope.user_name,birthday:$scope.user_birthday,gender:$scope.user_gender,id:res.data.data.id};
            $state.go('profile',details);
          }
       },function(error){
      alert(error);
...
SQLService.daysCount().then(function(result){
  ...
  $scope.test=function(){
    SQLService.getfm().then(function(result){
...
$scope.refresh=function(){
  ...
  $scope.deletePro=function(titl,index){
     $ionicPopup.confirm({
      title:'Confirm',
      ...
      if(res){
        ...
        $scope.proDetails.splice(index,1);
 ...
 $scope.next=function(){
  if(android || ios || windowsPh || ipad )
  {
    ...
    $cordovaGeolocation.getCurrentPosition(posOptions).then(function(position) {
      ...
      $ionicPopup.alert({
        title:"Location",
        template:"Please turn on your location and try again...",
        scope:$scope
      }).then(function(res){
